...
def test__medians_353(orig, mod):
    ...
    assert jnp.allclose(orig[0], mod)
...
def test_smooth_353QH_twice(caplog):
    ...
    assert jnp.allclose(hist_smooth, [1, 2, 3.25, 5])
    assert jnp.allclose(
    ...
    # with jnp.ndarray
    hist_smooth = smooth.smooth_353QH_twice(jnp.asarray([1, 3, 2, 5]))
    assert isinstance(hist_smooth, jnp.ndarray)
    assert hist_smooth.dtype == float
    assert jnp.allclose(hist_smooth, [1, 2, 3.25, 5])
    caplog.clear()

    assert jnp.allclose(smooth.smooth_353QH_twice([1, 3]), [1, 3])
