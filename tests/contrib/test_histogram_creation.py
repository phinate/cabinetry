...
def test_from_uproot(tmp_path, utils):
    ...
    assert jnp.allclose(yields, [1, 1, 2])
    assert jnp.allclose(stdev, [1, 1, 1.41421356])
    ...
    assert jnp.allclose(yields, [1, 1, 1])
    assert jnp.allclose(stdev, [1, 1, 1])
    ...
    assert jnp.allclose(yields, [2, 2, 6])
    assert jnp.allclose(stdev, [2, 2, 4.47213595])
    ...
    assert jnp.allclose(yields, [2, 2, 4])
    assert jnp.allclose(stdev, [2, 2, 2.82842712])
    ...
    yields, stdev = histogram_creation.from_uproot(fnames, treename, varname, bins)
    assert jnp.allclose(yields, [2, 2, 4])
    assert jnp.allclose(stdev, [1.41421356, 1.41421356, 2])
...
def test__bin_data():
    data = jnp.asarray([1.1, 2.2, 2.9, 2.5, 1.4])
    weights = jnp.asarray([1.0, 1.1, 0.9, 0.8, 1.5])
    bins = jnp.asarray([1, 2, 3])
    yields, stdev = histogram_creation._bin_data(data, weights, bins)
    assert jnp.allclose(yields, [2.5, 2.8])
    assert jnp.allclose(stdev, [1.80277564, 1.63095064])
